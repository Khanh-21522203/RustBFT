use crate::events::{ConsensusCommand, ConsensusEvent, Timeout, TimeoutKind, WalEntry, WalEntryKind};
use crate::proposer::select_proposer;
use crate::vote_set::VoteSet;
use rustbft_core::{
    Block, BlockHeader, Hash, Proposal, SignedProposal, SignedVote, ValidatorId, ValidatorSet,
    ValidatorSetPolicy, ValidatorUpdate, Vote, VoteType,
};
...
impl ConsensusCore {
    ...
    fn handle_vote(&mut self, sv: SignedVote) {
        ...
        match self.votes.insert_vote(&self.validator_set, sv) {
            Ok(()) => {}
            Err(crate::vote_set::VoteSetError::Duplicate) => return,
            Err(crate::vote_set::VoteSetError::Equivocation) => {
            ...
            Err(crate::vote_set::VoteSetError::HeightMismatch) => return,
            Err(crate::vote_set::VoteSetError::NonValidator) => return,
            Err(crate::vote_set::VoteSetError::UnexpectedVoteType) => return,
        }

...
use rustbft_consensus::events::{ConsensusCommand, ConsensusEvent};
use rustbft_node::contracts::ContractRuntime;
...
use rustbft_node::rpc::types::{JsonRpcRequest, ERR_BLOCK_NOT_FOUND, ERR_METHOD_NOT_FOUND};
use rustbft_node::execution::accounts::{AppState, ChainParams};
use rustbft_node::execution::executor::StateExecutor;
use rustbft_node::storage::block_store::BlockStore;
use rustbft_node::storage::state_store::StateStore;
use rustbft_node::storage::wal::WAL;
use rustbft_core::*;
use rustbft_core::codec::*;
...
fn test_block_store_save_and_load() {
    ...
    let store = BlockStore::open(&dir.join("blocks.db")).unwrap();
...
fn test_executor_and_storage_pipeline() {
    ...
    let block_store = BlockStore::open(&dir.join("blocks.db")).unwrap();
...
async fn test_router_storage_pipeline() {
    ...
    tx_cmd.send(ConsensusCommand::WriteWAL {
        entry: rustbft_node::storage::wal::WalEntry {
            height: 1,
            round: 0,
            kind: rustbft_node::storage::wal::WalEntryKind::Proposal,
            data: vec![1, 2, 3],
        },
    }).unwrap();
...
fn test_ed25519_sign_verify() {
    let (signing_key, verify_key) = crypto::generate_keypair();
    let message = b"hello rustbft";

    let signature = crypto::sign(&signing_key, message);
    assert!(crypto::verify(&verify_key, message, &signature), "valid signature should verify");

    // Tampered message should fail
    assert!(!crypto::verify(&verify_key, b"tampered", &signature), "tampered message should fail");
}
...
fn test_keypair_load_or_generate() {
    ...
    let (sk1, vk1) = crypto::load_or_generate_keypair(path_str).unwrap();

    // Second call loads same key
    let (sk2, vk2) = crypto::load_or_generate_keypair(path_str).unwrap();
...
fn test_wal_crash_recovery_simulation() {
    ...
    // Simulate: write proposal + prevote + precommit, then "crash" (drop WAL)
    {
        ...
        use rustbft_node::storage::wal::{WalEntry, WalEntryKind};
    ...
    assert_eq!(entries[0].kind, rustbft_node::storage::wal::WalEntryKind::Proposal);
    assert_eq!(entries[1].kind, rustbft_node::storage::wal::WalEntryKind::Prevote);
    assert_eq!(entries[2].kind, rustbft_node::storage::wal::WalEntryKind::Precommit);

...
use rustbft_node::storage::wal::{WAL, WalEntry, WalEntryKind, encode_wal_entry, decode_wal_entry};
use std::path::Path;
...
fn test_wal_entry_corrupt_checksum() {
    ...
    let mut encoded = encode_wal_entry(&entry);
    // Corrupt the last byte (checksum)
    let len = encoded.len();
    encoded[len - 1] ^= 0xFF;

    let result = decode_wal_entry(&encoded);

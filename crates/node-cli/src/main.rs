...
use rustbft_node::config::NodeConfig;
use rustbft_consensus::{
...
use rustbft_core::{SignedProposal, SignedVote, Validator, ValidatorId, ValidatorSet};
use rustbft_node::p2p::config::P2pConfig;
use rustbft_node::p2p::manager::P2pManager;
...
async fn main() -> anyhow::Result<()> {
    ...
    // 4. Storage (RocksDB)
    // -------------------------------------------------------
    let blocks_path = format!("{}/blocks.db", cfg.node.data_dir);
    let state_path = format!("{}/state.db", cfg.node.data_dir);
    ...
    info!(engine = "redb", data_dir = %cfg.node.data_dir, "Initializing storage");
...
fn init_logging(cfg: &rustbft_node::config::LoggingSection) {

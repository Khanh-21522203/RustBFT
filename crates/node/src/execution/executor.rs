...
use crate::execution::merkle::compute_state_root;
use crate::execution::tx::{decode_tx, DecodedTx, TxDecodeError};
use rustbft_core::{Address, Block, Hash, ValidatorId, ValidatorUpdate};
...
impl StateExecutor {
    ...
    pub fn execute_block(
        ...
        let mut pending_val_updates: Vec<crate::execution::tx::ValidatorUpdateTx> = Vec::new();
    ...
    fn end_block(
        &self,
        _state: &mut AppState,
        pending: &[crate::execution::tx::ValidatorUpdateTx],
    ...
    fn execute_one_tx(
        ...
        block: &Block,
        tx_bytes: &[u8],
        pending_val_updates: &mut Vec<crate::execution::tx::ValidatorUpdateTx>,
    ...
    fn exec_transfer(&self, state: &mut AppState, tx: crate::execution::tx::TransferTx) -> u64 {
    ...
    fn exec_deploy(
        ...
        contracts: &mut ContractRuntime,
        block: &Block,
        tx: crate::execution::tx::ContractDeployTx,
    ) -> Result<u64, ExecError> {
        ...
        let deploy_res = contracts
            .deploy(&tx.wasm)
            .map_err(|e: ContractError| ExecError::Contract(e.to_string()))?;
    ...
    fn exec_call(
        ...
        contracts: &mut ContractRuntime,
        block: &Block,
        tx: crate::execution::tx::ContractCallTx,
    ) -> Result<u64, ExecError> {
        let intrinsic = self
            .gas
            .base_tx
